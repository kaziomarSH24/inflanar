...
class PaymentController extends Controller
{
    ...
    public function applyCampaign(Request $request, $slug){
        ...
        $influencer_id = $service->influencer_id; //here influencer means business provider
        $client_id = $user->id; //here client means influencer

        //check can create order
        $response = $this->canOrder($user->id);
        if($response->status() == 403 ){
            $notification= $response->getData()->message;
            return response()->json(['status' => 'error' , 'message' => $notification]);
        }

        $order = $this->create_order($user, $service, $booking_info, $influencer_id, $client_id, 'n/a', 'success','n/a');

        $notification = trans('admin_validation.You\'ve successfully applied for this campaign! Good luck!');
