...
@section('frontend-content')
                    ...
                    <div class="col-lg-9 col-12 mg-top-30">
                        <div class="inflanar-sdetail">
                            ...
                            <div class="inflanar-sdetail__thumb">
                                <img src="{{ asset($service_info->thumbnail_image) }}" alt="#">
                            </div>
                            <!-- Service Content -->
                            <div class="inflanar-sdetail__content">
                                <h2 class="inflanar-sdetail__title mg-btm-20">{{__('admin.Service Create Information')}}</h2>
								...
								<div class="inflanar-package-info__group">
									...
									<div class="inflanar-package-info__single">
										<p><span><b>{{__('admin.Extra Service')}}</b></span> <span><b class="extra_service_price">
                                            {{ currency($service_info->extra_total) }}
                                        </b></span></p>
                                        @if (is_array($service_info->ids))
                                        @foreach ($service_info->ids as $extra_index => $extra_id)
										<p><span>(+) {{ $service_info->names[$extra_index] }}</span> <span>
                                            {{ currency($service_info->prices[$extra_index]) }}
                                        </span></p>
                                        @endforeach
                                        @endif
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            <form action="{{ route('pay-via-bank', $service_info->slug) }}" method="POST">
                @csrf
    ...
    <div class="modal fade" id="stripeModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" action="{{ route('pay-via-stripe', $service_info->slug) }}" method="POST" class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{ $stripe->stripe_key }}" id="payment-form">
                    @csrf
